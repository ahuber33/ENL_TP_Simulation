...
ENLTPSimTrackingAction::ENLTPSimTrackingAction(){}
ENLTPSimTrackingAction::~ENLTPSimTrackingAction(){}
void ENLTPSimTrackingAction::PreUserTrackingAction(const G4Track* aTrack){
  ...
  if( partname == "opticalphoton" && aTrack->GetUserInformation() == 0 ){
	  ...
	  theTrack->SetUserInformation(new ENLTPSimTrackInformation(aTrack));
      }
...
void ENLTPSimTrackingAction::PostUserTrackingAction(const G4Track* aTrack){
  ...
  if(partname == "opticalphoton"){
	  ...
	  if(nSeco > 0){
		  ...
		  if(cproc == "OpWLS") {
		      (*secondaries)[i]->SetUserInformation(new ENLTPSimTrackInformation(aTrack));
		  }
	      }
	  }
      }
